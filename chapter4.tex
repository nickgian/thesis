...
\ch{Btw, you should remind the reader that everything in this chapter
  was formalized in Coq, and the development is available upon request
  or whatever we might even make it public before you submit.}
\nick{rephrased the intro, old one in comment. Still needs some iterations.}

In this chapter we develop our main results. In particular, we used
Coq to prove a property capturing the notion of \CFI, similar to what
was proposed by Abadi \ETAL in \cite{AbadiBEL09}, for the concrete
machine running monitor code that implements the micro-policy of
\cref{sec:cfi_fine}. 
...
concrete machine. Furthermore this preservation theorem, allowed us to
obtain a proof for \CFI for the Concrete machine by using the
micro-policies framework of \cref{sec:framework} in order to easily
obtain a $\lbrace 0,1 \rbrace$-backward simulation between the
Concrete and the Symbolic machine. As a result the proof effort
required, was considerably reduced, as we essentially had to do most
of our reasoning at the Symbolic machine level.
...
by construction and therefore the proof of the \CFI property for this
Abstract machine is trivial. Additionally we proved a 1-backward
simulation between the Symbolic and the Abstract machine, which
allowed us to invoke the generic preservation theorem in order to
...
Finally, in addition to the easily obtainable backward refinement between
the Concrete and the Abstract machine, we prove a 1-forward
simulation between the Abstract and the Symbolic machine. These refinement
proofs provide us with additional assurance in the correctness of
our micro-policy.
  ...
  \begin{center}
    \begin{tikzpicture}
      [machine/.style={rectangle,draw=teal!80,text=black,very thick,minimum height=1cm},
       cfi/.style={rectangle,draw,thick,dashed,draw=teal!80,text=black},
       >=stealth]
      ...
      \node [rectangle,draw,dashed,-,thick,fill=teal!20,rounded corners]
            (symbolic rules) [right=of symbolic label] {CFI Micro-Policy};
      \begin{pgfonlayer}{background}
        \node [machine,fill=teal!10,fit={(symbolic label) (symbolic rules)}]
              (symbolic) {};
      \end{pgfonlayer}
      \node [cfi,dashed,right=of symbolic,fill=teal!10] (symbolic cfi) {CFI Property};

      %abstract cfi
      \node [cfi,fill=teal!10] (abstract cfi)  at (abstract -| symbolic cfi) {CFI Property};
      ...
      \begin{pgfonlayer}{background}
        \node [machine,fill=teal!10,fit={(concrete label) (fault handler)}]
              (concrete) {};
      \end{pgfonlayer}
      \node [cfi,dashed,fill=teal!10] (concrete cfi) at (concrete -| symbolic cfi) {CFI Property};
    ...
    \end{tikzpicture}
  \end{center}
\caption{Diagram explaining proof structure\ch{This diagram would
  already be very useful in the intro}}
...
\infrule[Jump]{
  ...
  \ii{ok} = (\pc,\pc') \in \Jm
    ...
    \acfistat{\imem}{\dmem}{\reg'}{\pc'}{\ii{ok}}}}
\caption{Step rule\chrev{s}\ch{plural of rule is rules}
  for Jump and Jal instruction of abstract machine}
\end{figure}
...
class of identifiers \cref{id_class}.
