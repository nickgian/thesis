  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw [thick] (symbolic) to (symbolic cfi);
        \draw [->,thick]
              (abstract cfi)
              to node [right,text width=3cm] {Preserved\\by backward \\simulation}
              (symbolic cfi);
...
In \cref{abs_semantics} we define the operational semantics of the Abstract Machine.
...
\begin{figure}[htpb]
\infrule[Nop]{
  \imem[\pc] = i \andalso \ii{decode}~i = \ii{Nop}
  }{\stepn{\acfistat{\imem}{\dmem}{\reg}{\pc}{\ii{true}}}{
    \acfistat{\imem}{\dmem}{\reg}{\pc+1}{\ii{true}}}}
\bigskip

\infrule[Const]{
  \imem[\pc] = i \andalso \ii{decode}~i = \ii{Const}~n~r \\
  \reg' = \upd{\reg}{r}{n} \andalso
  }{\stepn{\acfistat{\imem}{\dmem}{\reg}{\pc}{\ii{true}}}{
    \acfistat{\imem}{\dmem}{\reg'}{\pc+1}{\ii{true}}}}
\bigskip

\infrule[Mov]{
  \imem[\pc] = i \andalso \ii{decode}~i = \ii{Mov}~r_p~r_s \\
  \rd{\reg}{r_p} {=} w_p \andalso
  \reg' = \upd{\reg}{r_s}{w_p}
  }{\stepn{\acfistat{\imem}{\dmem}{\reg}{\pc}{\ii{true}}}{
    \acfistat{\imem}{\dmem}{\reg'}{\pc+1}{\ii{true}}}}
\bigskip
...
\infrule[Binop]{
  \imem[\pc] = i \andalso \ii{decode}~i = \ii{Binop}~op~r_p~r_s~r_t \\
  \rd{\reg}{r_p} {=} w_p \andalso
  \rd{\reg}{r_s} {=} w_s \andalso
  \reg' = \upd{\reg}{r_t}{w_p~op~w_s}
  }{\stepn{\acfistat{\imem}{\dmem}{\reg}{\pc}{\ii{true}}}{
    \acfistat{\imem}{\dmem}{\reg'}{\pc+1}{\ii{true}}}}
\bigskip

\infrule[Load]{
  \imem[\pc] = i \andalso \ii{decode}~i = \ii{Load}~r_p~r_s \\
  \rd{\reg}{r_p} {=} w_p \andalso
  \rd{\imem}{w_p} {=} w \lor \rd{\dmem}{w_p} {=} w \\
  \reg' = \upd{\reg}{r_s}{w}
  }{\stepn{\acfistat{\imem}{\dmem}{\reg}{\pc}{\ii{true}}}{
    \acfistat{\imem}{\dmem}{\reg'}{\pc+1}{\ii{true}}}}
\bigskip
...
\infrule[Store]{
  \imem[\pc] = i \andalso \ii{decode}~i = \ii{Store}~r_p~r_s \\
  \rd{\reg}{r_p} {=} w_p \andalso
  \rd{\reg}{r_s} {=} w_s \\
  \dmem' = \upd{\dmem}{w_p}{w_s}
  }{\stepn{\acfistat{\imem}{\dmem}{\reg}{\pc}{\ii{true}}}{
    \acfistat{\imem}{\dmem'}{\reg}{\pc+1}{\ii{true}}}}
\bigskip
...
\infrule[Jal]{
  \imem[\pc] = i \andalso \ii{decode}~i = \ii{Jal}~r \\
  \rd{\reg}{r} = \pc' \andalso
...
\infrule[Bnz]{
  \mem[\pc] = i \andalso \ii{decode}~i = \ii{Bnz}~r~n \andalso
  \rd{\reg}{r} {=} w \\
...
\begin{proof}
  ...
  \item \textbf{Base Case} In this case the execution trace is made up
    ...
    \item \textbf{Attacker Step} By \cref{attacker_no_v} we note that
      ...
      by \SUCC{A}. Thus in this case the whole trace has \CFI
...
it is the most straightforward. It is worth to note that bugs in the
...
\infrule[IndirectFlows]{
  ...
  \astat{\mem}{\reg}{\pc'}{\extra}) \in \SUCCm{S}}
\bigskip
...
\begin{definition}[Registers Tagged]\label{registers_tagged}
...
\vspace{-\smallskipamount}
 \begin{center}
    \begin{tikzpicture}
      ...
      edge [above,dashed] node [below] {} (m-1-2)
      (m-2-1.east|-m-2-2) edge [] node {}
      node {} (m-2-2)
...
\label{simulation_SA}
\Cref{simulation_LH} holds for the Symbolic (low-level) and the
...
corresponding lemma for the data memory. Its proof was again more
...
In this section, we develop the preservation theorem that we used,
    ...
    \begin{figure}[H]
      \begin{center}
        \begin{tikzpicture}
          ...
          \path[->]
          (m-1-1) edge [snake it,-] node {} (m-2-1)
          edge [above] node [above,very near end,dashed,font=\small] {n} (m-1-2)
    ...
    \begin{figure}[H]
      \begin{center}
        \begin{tikzpicture}
          ...
          (m-2-1.east|-m-2-2) edge [above,font=\small, very near end] node {n}
          node {} (m-2-2)
          (m-1-1) edge [snake it,dashed,-] node {} (m-2-2);
        \end{tikzpicture}
      \end{center}
    ...
    \begin{figure}[H]
      \begin{center}
        \begin{tikzpicture}
          ...
          \path[->]
          (m-1-1) edge [snake it,-] node {} (m-2-1)
          edge [above] node [above,very near end,dashed,font=\small] {a} (m-1-2)
