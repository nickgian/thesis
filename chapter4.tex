...
\cref{sec:cfi_fine}\ch{can't instantiate the concrete
  machine with symbolic rules, there is a mismatch here,
  you probably mean: ``the concrete machine running monitor 
  code implementing the micro-policy in \cref{sec:cfi_fine}''}
\ii{simulates} an abstract machine that has \CFI
by construction.\ch{Not simulation is the main result;
  simulation is just a way of proving CFI for the concrete machine,
  which is the main result and should come first.}
We do this proof by using the symbolic machine as an
...
when the attacker ca tamper all data on the machine, similarly to what is
...
mentioned before, monitor services, execute in a protected\ch{a protected what?}
by the
...
\cref{cfi_machine}, with the abstract machine and prove that the abstract machine
has \CFI according to \cref{cfi}.
We first prove a helpful lemma\ch{, which intuitively says that ...}.
