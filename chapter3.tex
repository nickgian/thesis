...
spread among security researchers\ch{This sounds awful}.
...
first proposed a technique to enforce \CFI based on IRMs.
...
The \chrev{security of the}
above \chrev{coarse-grained}
techniques are evaluated in \cite{outofcontrol_ieeesp2014} where
...
\section{Micro-Policy for Fine-Grained Control-Flow Integrity}\label{sec:cfi_fine}
...
\ch{Shrink and polish this}\nick{done} We follow the standard
approach, by designing a composed micro-policy that enforces \NXD,
\NWC and \CFI\ch{I don't think this is standard in any way.
  Abadi at al were {\bf assuming} NXD and NWC from other mechanisms,
  not enforcing it themselves.}.
We considered designs that lifted the \NXD and \NWC
restrictions but we rejected them, as there did not seem to be any
considerable advantages (\IE{ compatibility with self-modifying
  programs, JIT compilers, \ETC}). Moreover unlike other \CFI
...
one location in the memory). One \strikeout{simply}\chrev{simple}\ch{simply
  is an adverb, you need an adjective here, so simple}
