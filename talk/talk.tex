...
Constraints all execution paths by a statically computed control-flow graph.
\begin{itemize} 
...
\item Proved a two-way refinement between the concrete machine running
  ...
  construction \pause
\end{itemize}
\textbf{Challenge:} Avoid direct proof of CFI property on low-level machine
}
...
\begin{block}{Key Ideas}
  ...
  \setlength{\itemsep}{1pt}
    ...
    \item Check invariants on meta-data and propagate them accordingly
  \end{itemize}
\end{block}
...
\begin{itemize}
  \setlength{\itemsep}{1pt}
  \item Models above hardware-software mechanism
...
\begin{block}{Initial tagging scheme}
  \begin{itemize}
  \item Sources/targets of indirect control-flows tagged using their address (\textit{addr})
    \INSTR{\textit{addr}}
...
\begin{itemize}
\item Wraps symbolic tags around tags for self-protection
  \begin{itemize}
    \item \textit{User (enc ut)} where $ut \in \mathcal{T}$
    \item \textit{Monitor} for policy-monitor data
    \item \textit{Entry (enc ut)} for calling privileged monitor services from user code
    \end{itemize}
