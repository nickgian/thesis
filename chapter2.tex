...
Store instruction the tag on the memory access (MR), \IE the tag on the
...
above rules can be read as ``Execution is only allowed if the tag of
the current instruction is \INSTRname. Furthermore if the opcode
of the current instruction is Store, execution should be allowed only
if the tag on the memory being overwritten is \DATA. In that case
the tag of the new content in the memory should remain \DATA. For all other
opcodes the result tags are indifferent to the enforced policy.''

%original text
% ``If the tag on the current instruction is
...
of micro-policies \cite{pump_asplos2015}, using software to describe
